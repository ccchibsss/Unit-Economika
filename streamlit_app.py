...
def render_sidebar() -> DataPipeline:
    with st.sidebar:
        st.markdown("### 📑 Главное меню")
        st.markdown("---")
        
        st.markdown("### 🔑 Авторизация и API")
        ...
        st.markdown("---")
        st.markdown("### ⚖️ Финансовые параметры")
        ...
        st.markdown("---")
        st.markdown("### 🛠️ Системные инструменты")
        if st.button("🗑️ Очистить кэш и перезапустить"):
            st.cache_data.clear()
            st.session_state.clear()
            st.rerun()
        st.markdown("*Версия ядра: 23.4.0*")
