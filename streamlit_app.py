...
class HighVolumeAutoPartsCatalog:
    ...
    def build_export_query(self, selected_columns=None, include_prices=True, apply_markup=True):
        ...
        # ✅ v100.23: ИСПРАВЛЕННЫЕ CTE - добавлена колонка related_oe_number_norm
        ctes = f"""
...
# ============================================================================
# БЛОК 17: UI функции каталога (ПОЛНАЯ ВЕРСИЯ v100.5.4)
# ============================================================================
# ✅ ИСПРАВЛЕНИЯ v100.5.4:
# 1. Исправлен st.rerun() - теперь используется st.rerun() только в безопасных местах
# 2. Добавлена кнопка сброса БД без ошибок
...
""")
    ...
    # ✅ Используем st.sidebar для меню каталога с уникальным ключом
    with st.sidebar:
        st.markdown("---")
...
def show_catalog_upload(catalog):
    ...
    # ✅ БЕЗОПАСНАЯ КНОПКА СБРОСА БД
    col1, col2 = st.columns([3, 1])
    with col1:
        st.warning("⚠️ Если в габаритах отображаются даты (22.июл, 08.фев) - нажмите кнопку сброса БД")
    with col2:
        if st.button("🗑️ СБРОСИТЬ БД", type="secondary", key="reset_db_btn"):
            try:
                if catalog.delete_database_file():
                    st.success("✅ База удалена! Перезагрузите страницу и загрузите данные заново.")
                    # ✅ БЕЗОПАСНЫЙ rerun - используем st.rerun() только если он доступен
                    try:
                        st.rerun()
                    except Exception:
                        # Если st.rerun() не работает, просто показываем сообщение
                        st.info("🔄 Пожалуйста, обновите страницу вручную (F5)")
            except Exception as e:
                st.error(f"❌ Ошибка сброса БД: {e}")
